...
from flask import current_app, render_template # Import current_app for config, render_template for HTML body
...
def send_schedule_update_email(employee, shifts):
    ...
    if not (employee and employee.email):
        log.warning(f"Attempted to send schedule email to employee ID {employee.id if employee else 'N/A'} but email address is missing.")
        return False

    if not shifts:
        log.info(f"No shifts to notify for employee {employee.name} ({employee.email}). Email not sent.")
    ...
    try:
        ...
        log.error(f"Error sending schedule email to {employee.email}: {e}", exc_info=True) # Log full exception
