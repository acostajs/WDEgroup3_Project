...
def delete_employee(employee_id):
    ...
    try:
        ...
        if has_shifts or has_logs:
            ...
            relations = []
            if has_shifts: relations.append("shifts")
            if has_logs: relations.append("performance logs")
            flash(f'Cannot delete employee "{employee.name}" because they have existing { " and ".join(relations) }. Please reassign or delete associated records first.', 'danger')
        else:
...
def add_performance_log():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if existing_log:
                flash(f'Error: A performance log already exists for {employee.name} in {log_date.strftime("%B %Y")}. Only one per month allowed.', 'danger')
                return render_template('admin/add_performance.html', title='Log Performance', form=form)
        ...
        try:
            ...
            db.session.commit()
            flash(f'Performance logged successfully for {employee.name} on {log_date}.', 'success')
...
def performance_dashboard():
    ...
    try:
        ...
        return render_template(
            'admin/performance_dashboard.html',
